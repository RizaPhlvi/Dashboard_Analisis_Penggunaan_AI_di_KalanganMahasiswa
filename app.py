...
st.markdown("""
    ...
    div[data-testid="stMetricValue"] { color: #38BDF8; font-weight: 700; font-size: 32px; }
    div[data-testid="stMetricLabel"] { color: #94A3B8; }
    [data-testid="stVerticalBlockBorderWrapper"] > div { background-color: #1E293B !important; border-color: #334155 !important; border-radius: 12px; }
    .header-box { background: linear-gradient(135deg, #1D4ED8 0%, #3B82F6 100%); padding: 30px; border-radius: 15px; color: white; box-shadow: 0 10px 25px rgba(59, 130, 246, 0.2); margin-bottom: 20px; border: 1px solid #3B82F6; }
...
COLORS = {
    'primary': '#3B82F6', 'secondary': '#0EA5E9', 'success': '#10B981',
    'danger': '#EF4444', 'warning': '#F59E0B', 'light': '#38BDF8', 'slate': '#64748B'
}
...
with st.sidebar:
    ...
    filter_prodi = st.multiselect("Filter Program Studi", options=df_raw['Prodi'].unique(), default=df_raw['Prodi'].unique())
    filter_semester = st.multiselect("Filter Semester", options=sorted(df_raw['Semester'].unique()), default=sorted(df_raw['Semester'].unique()))
    
    st.markdown("━━━━━━━━━━━━━━━━━━━━━━")
    st.markdown("**🔮 PROFIL SIMULATOR**")
    sim_tugas = st.slider("Porsi Bantuan AI Anda:", 0, 10, 6)
    if sim_tugas > 5: st.error("⚠️ Risiko Ketergantungan Tinggi")
    else: st.success("✅ Ketergantungan Aman")
...
with kpi1:
    with st.container(border=True):
        st.metric("👥 Total Sampel", f"{len(df)} Mhs")
        st.progress(1.0)
with kpi2:
    with st.container(border=True):
        st.metric("⏱️ Durasi Rata-rata", f"{df['Jam_per_Hari'].mean():.1f} Jam")
        st.progress(min(df['Jam_per_Hari'].mean()/10.0, 1.0))
with kpi3:
    with st.container(border=True):
        st.metric("📝 Bantuan Tugas", f"{df['Porsi_Tugas_AI'].mean():.1f} / 10")
        st.progress(df['Porsi_Tugas_AI'].mean()/10.0)
with kpi4:
    with st.container(border=True):
        st.metric("⭐ Efektivitas", f"{df['Skor_Efektivitas'].mean():.2f} / 5")
        st.progress(df['Skor_Efektivitas'].mean()/5.0)

st.markdown("<br>", unsafe_allow_html=True)

tab1, tab2, tab3 = st.tabs(["📊 Eksplorasi Deskriptif", "🔗 Hubungan & Probabilitas", "🎲 Monte Carlo"])

# ==========================================
# TAB 1: EKSPLORASI DESKRIPTIF (Ditambah Visual Laporan)
# ==========================================
with tab1:
    with st.container(border=True):
        ...
        fig_hero = px.bar(trend_data, x='Frekuensi', y='Jumlah', text='Jumlah', color='Frekuensi', color_discrete_sequence=[COLORS['primary'], COLORS['secondary'], COLORS['light'], COLORS['slate']], template=PLOTLY_TEMPLATE)
    ...
    # Baris 2: Durasi & Porsi Tugas
    col1, col2 = st.columns(2)
    with col1:
        with st.container(border=True):
            st.markdown("#### ⏳ Distribusi Durasi (Jam/Hari)")
            fig_hist = px.histogram(df, x='Jam_per_Hari', nbins=8, marginal="box", color_discrete_sequence=[COLORS['secondary']], template=PLOTLY_TEMPLATE)
    ...
    with col2:
        with st.container(border=True):
            ...
            fig_porsi = px.histogram(df, x='Porsi_Tugas_AI', text_auto=True, color_discrete_sequence=[COLORS['primary']], template=PLOTLY_TEMPLATE)
    ...
    # Baris 3: Efektivitas & Persepsi Peningkatan
    col3, col4 = st.columns(2)
    with col3:
        with st.container(border=True):
            st.markdown("#### ⭐ Distribusi Skor Efektivitas Belajar")
            fig_skor = px.histogram(df, x='Skor_Efektivitas', text_auto=True, color_discrete_sequence=[COLORS['success']], template=PLOTLY_TEMPLATE)
    ...
    with col4:
        with st.container(border=True):
            ...
            fig_nilai = px.histogram(df, x='Peningkatan_Nilai', text_auto=True, color_discrete_sequence=[COLORS['warning']], template=PLOTLY_TEMPLATE)
...
# ==========================================
# TAB 2: HUBUNGAN & PROBABILITAS (Ditambah Visual Laporan)
# ==========================================
with tab2:
    ...
    with col5:
        with st.container(border=True):
            ...
            fig_prob = px.bar(prob_df, x='Is_Ketergantungan_Tinggi', y='Persentase', color='Kesulitan', barmode='stack', text_auto='.1f', color_discrete_map={'Ya': COLORS['danger'], 'Tidak': COLORS['success']}, template=PLOTLY_TEMPLATE)
            fig_prob.update_layout(height=350, margin=dict(t=20, b=20, l=0, r=0))
    ...
    with col6:
        with st.container(border=True):
            ...
            fig_heat = px.imshow(corr_matrix, text_auto=".3f", aspect="auto", color_continuous_scale="Blues_r", origin="lower", template=PLOTLY_TEMPLATE)
            fig_heat.update_layout(height=350, margin=dict(t=20, b=20, l=0, r=0))
    ...
    with col7:
        with st.container(border=True):
            ...
            # Scatter Plot dengan Garis Regresi Linier Manual
            z = np.polyfit(df['Porsi_Tugas_AI'], df['Skor_Efektivitas'], 1)
            p = np.poly1d(z)
            df_sorted = df.sort_values('Porsi_Tugas_AI')
            
            fig_scatter = px.scatter(df, x='Porsi_Tugas_AI', y='Skor_Efektivitas', opacity=0.7, template=PLOTLY_TEMPLATE)
            fig_scatter.update_traces(marker=dict(size=12, color=COLORS['secondary']))
            fig_scatter.add_trace(go.Scatter(x=df_sorted['Porsi_Tugas_AI'], y=p(df_sorted['Porsi_Tugas_AI']), mode='lines', name='Trendline', line=dict(color=COLORS['danger'], width=3)))
    ...
    with col8:
        with st.container(border=True):
            ...
            fig_box = px.box(df, x='Porsi_Tugas_AI', y='Skor_Efektivitas', color_discrete_sequence=[COLORS['primary']], template=PLOTLY_TEMPLATE)
    ...
    with st.container(border=True):
        ...
        fig_cp = px.bar(cp_grouped, x='Porsi_Tugas_AI', y='Tingkat_Copy_Paste', text_auto='.2f', color='Tingkat_Copy_Paste', color_continuous_scale="Oranges", template=PLOTLY_TEMPLATE)
    ...
    with st.container(border=True):
        ...
        with mc_c2:
            if st.session_state.get('run_mc', False):
                ...
                col_m3.metric("95% CI", f"{ci_low:.2f} - {ci_high:.2f}")
